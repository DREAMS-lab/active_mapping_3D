"""NBV reconstruction: fully adaptive anisotropy-based capture + offline 3DGS.

Launches:
  1. ros_gz_bridge (camera + gimbal topics)
  2. Static TF publisher (map frame)
  3. NBV active mapper node (seed + iterative NBV scoring + adaptive flight)
...
def generate_launch_description():
    nbv_share = get_package_share_directory('nbv')
    # Derive workspace root: <ws>/install/nbv/share/nbv -> <ws>
    ws_root = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.dirname(nbv_share))))
    ...
    active_mapper_script = os.path.join(
        nbv_share, 'scripts', 'active_mapper_node_nbv.py')
