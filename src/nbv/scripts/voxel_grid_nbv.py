...
from gaussian_model_nbv import FX, FY, CX, CY
...
class VoxelGrid:
    ...
    def get_low_view_centers(self, min_views=2):
        """Return (M, 3) float32 array of voxel centers with < min_views observations.

        Only includes occupied voxels (seen at least once but under-observed).
        """
        mask = self.occupied & (self.n_views < min_views)
        indices = np.argwhere(mask)  # (M, 3)
        if len(indices) == 0:
            return np.zeros((0, 3), dtype=np.float32)
        centers = self.origin + (indices + 0.5) * self.resolution
