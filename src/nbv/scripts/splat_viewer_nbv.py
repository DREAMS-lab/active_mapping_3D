...
from gaussian_model_nbv import FX, FY, CX, CY, W, H
...
class SplatViewer:
    ...
    def __init__(self, rock_center, bbox_size=2.0, device=None):
        ...
        # Viewer renders at full camera resolution
        self._viewer_scale = 1
        vs = self._viewer_scale
        self._viewer_w = W // vs
        self._viewer_h = H // vs
        self.K = torch.tensor([
            [FX / vs, 0, CX / vs], [0, FY / vs, CY / vs], [0, 0, 1],
    ...
    def _project_point(self, pt_world, viewmat):
        ...
        vs = self._viewer_scale
        u = (FX / vs) * p[0] / p[2] + (CX / vs)
        v = (FY / vs) * p[1] / p[2] + (CY / vs)
        return int(u), int(v)
    ...
    def _render_frame(self):
        ...
        if snap is None:
            img = np.zeros((self._viewer_h, self._viewer_w, 3), dtype=np.uint8)
            cv2.putText(img, "Waiting for data...",
                        (self._viewer_w // 2 - 120, self._viewer_h // 2),
        ...
        with torch.no_grad():
            renders, _, _ = gsplat.rasterization(
                ...
                viewmats=vm[None],
                Ks=self.K[None],
                width=self._viewer_w,
                height=self._viewer_h,
                render_mode='RGB',
                packed=True,
                near_plane=0.3,
                far_plane=12.0,
    ...
    def run(self):
        ...
        while self._running:
            ...
            cv2.putText(img, controls, (10, self._viewer_h - 10),
