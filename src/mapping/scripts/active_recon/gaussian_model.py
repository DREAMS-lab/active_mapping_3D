...
class GaussianModel3DGS:
    ...
    def get_snapshot(self):
        ...
        Returns dict with means, quats, scales, opacities, colors as CPU tensors,
        ...
        with torch.no_grad():
            return {
                ...
                'quats': F.normalize(self.quats.detach(), dim=-1),
                'scales': torch.exp(self.scales.detach()),
                'opacities': torch.sigmoid(self.opacities.detach()),
                'colors': torch.sigmoid(self.colors.detach()),
