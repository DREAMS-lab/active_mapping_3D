...
from gaussian_model_pu import GaussianModel3DGS, FX, FY, CX, CY, W, H, W_NBV, H_NBV
from splat_viewer_pu import SplatViewer
from voxel_grid_pu import VoxelGrid
from rviz_publisher_pu import RVizPublisher
...
class ActiveMapperNode(Node):
    def __init__(self):
        ...
        from nbv_planner_pu import NBVPlanner
        ...
        # -- Output directory --
        # Workspace root: <ws>/install/nbv_pos_uncert/share/nbv_pos_uncert/scripts/this.py -> <ws>
        self.ws_root = os.path.dirname(os.path.dirname(os.path.dirname(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))
        data_base = os.path.join(self.ws_root, 'data', 'nbv_pos_uncert')
        ...
        # -- Components --
        # Save original rock position (before NED calibration rotation).
        # Depth backprojection lands points at this position regardless of
        # PX4 heading offset, so voxels and Gaussian bbox must use it.
        ...
        self.pos = None
        self.odom = None  # VehicleOdometry — has position_variance
        self.status = None
        self.battery = None
        ...
        # Bounding box published after DETECT phase (not here)
    ...
    def _set_gimbal_to_rock(self):
        """Compute gimbal pitch to center the rock in the camera frame.

        Called ONCE when settled at a viewpoint. Aims at the geometric
        center of the rock (not the top) so the full bounding box is
        visible regardless of approach angle. Minimum -10° to avoid
        seeing landing legs at low elevations.
        """
        dx = self.rock_ned[0] - self.pos.x
        dy = self.rock_ned[1] - self.pos.y
        dz = self.rock_ned[2] - self.pos.z  # aim at center, not top
        horiz = math.sqrt(dx * dx + dy * dy)
        if horiz < 0.1:
            # Directly above rock — look straight down
            self.gimbal_pitch_rad = math.radians(-90)
        else:
            pitch = -math.atan2(dz, horiz)
            self.gimbal_pitch_rad = max(pitch, math.radians(-90))
            self.gimbal_pitch_rad = min(self.gimbal_pitch_rad,
                                        math.radians(-10))
        self.get_logger().debug(
            f'Gimbal pitch set to {math.degrees(self.gimbal_pitch_rad):.1f}°')

    def _backproject_depth_to_world(self, depth_np, mask):
        """Backproject masked depth pixels to world coordinates."""
        vs, us = np.where(mask)
        ds = depth_np[vs, us]
        ...
        yaw = math.atan2(self.rock_y - py, self.rock_x - px)
    ...
    def _detect_from_above(self, depth_np):
        """Phase 1: overhead view — get x, y center and top surface z."""
        valid = np.isfinite(depth_np) & (depth_np > 0.3) & (depth_np < 12.0)
        if valid.sum() < 100:
            self.get_logger().warn('DETECT_ABOVE: too few valid pixels')
            return
        ground_depth = np.percentile(depth_np[valid], 85)
        object_mask = valid & (depth_np < (ground_depth - 0.15))
        n_obj = object_mask.sum()
        if n_obj < 50:
            self.get_logger().warn(f'DETECT_ABOVE: only {n_obj} object pixels')
            return
        pts = self._backproject_depth_to_world(depth_np, object_mask)
        cx = float(np.median(pts[:, 0]))
        cy = float(np.median(pts[:, 1]))
        z_top = float(np.median(pts[:, 2]))
        # Horizontal extent
        x_extent = float(np.percentile(pts[:, 0], 95) - np.percentile(pts[:, 0], 5))
        y_extent = float(np.percentile(pts[:, 1], 95) - np.percentile(pts[:, 1], 5))
        self._detect_data['cx'] = cx
        self._detect_data['cy'] = cy
        self._detect_data['z_top'] = z_top
        self._detect_data['x_extent'] = x_extent
        self._detect_data['y_extent'] = y_extent
        self._detect_data['n_pixels_above'] = n_obj
        self.get_logger().info(
            f'DETECT_ABOVE: center=({cx:.2f},{cy:.2f}), z_top={z_top:.2f}, '
            f'extent=({x_extent:.2f}x{y_extent:.2f}), {n_obj} pixels')

    def _detect_from_side(self, depth_np):
        """Phase 2: side view — get vertical extent."""
        valid = np.isfinite(depth_np) & (depth_np > 0.3) & (depth_np < 12.0)
        if valid.sum() < 100:
            self.get_logger().warn('DETECT_SIDE: too few valid pixels')
            return
        # From the side, rock pixels are closer than background
        bg_depth = np.percentile(depth_np[valid], 85)
        object_mask = valid & (depth_np < (bg_depth - 0.15))
        n_obj = object_mask.sum()
        if n_obj < 50:
            self.get_logger().warn(f'DETECT_SIDE: only {n_obj} object pixels')
            return
        pts = self._backproject_depth_to_world(depth_np, object_mask)
        z_min = float(np.percentile(pts[:, 2], 5))   # bottom of rock (NED)
        z_max = float(np.percentile(pts[:, 2], 95))   # top of rock (NED)
        z_extent = abs(z_max - z_min)
        self._detect_data['z_min'] = z_min
        self._detect_data['z_max'] = z_max
        self._detect_data['z_extent'] = z_extent
        self._detect_data['n_pixels_side'] = n_obj
        self.get_logger().info(
            f'DETECT_SIDE: z=[{z_max:.2f},{z_min:.2f}], '
            f'height={z_extent:.2f}m, {n_obj} pixels')

    def _finalize_detection(self):
        """Combine overhead + side views into final bbox."""
        d = self._detect_data
        cx = d.get('cx', self.rock_x)
        cy = d.get('cy', self.rock_y)
        z_top = d.get('z_top', self.rock_ned[2])
        z_min = d.get('z_min', z_top + 0.5)  # fallback: assume 0.5m height
        z_max = d.get('z_max', z_top)
        cz = (z_max + z_min) / 2.0  # vertical center

        # Conservative bbox: max of horizontal and vertical extents + margin
        x_ext = d.get('x_extent', 1.0)
        y_ext = d.get('y_extent', 1.0)
        z_ext = d.get('z_extent', 0.5)
        margin = 0.5
        bbox_size = max(x_ext, y_ext, z_ext) + margin
        bbox_size = max(bbox_size, 1.0)  # minimum 1m
        bbox_size = min(bbox_size, 4.0)  # maximum 4m

        old_center = self.rock_ned.copy()
        dist = math.sqrt((cx - old_center[0])**2 +
                         (cy - old_center[1])**2 +
                         (cz - old_center[2])**2)

        if dist > 5.0:
            self.get_logger().warn(
                f'DETECT: center ({cx:.2f},{cy:.2f},{cz:.2f}) is '
                f'{dist:.1f}m from param — using launch params')
            return

        # Update everything
        import torch
        self.rock_ned = np.array([cx, cy, cz], dtype=np.float32)
        self.rock_ned_original = self.rock_ned.copy()
        self.rock_x = cx
        self.rock_y = cy
        self.bbox_size = float(bbox_size)

        self.planner = self.planner.__class__(
            rock_ned=self.rock_ned.tolist(),
            radius=self.radius,
            bbox_size=self.bbox_size,
            device=self.gs_model.device)
        self.voxels = self.voxels.__class__(
            self.rock_ned, self.bbox_size, self.voxel_res)
        self.gs_model.bbox_center = self.rock_ned
        half = self.bbox_size / 2
        self.gs_model.bbox_min = torch.tensor(
            self.rock_ned - half, device=self.gs_model.device)
        self.gs_model.bbox_max = torch.tensor(
            self.rock_ned + half, device=self.gs_model.device)

        self.get_logger().info(
            f'DETECT: final bbox center=({cx:.2f},{cy:.2f},{cz:.2f}), '
            f'size={bbox_size:.2f}m, {dist:.2f}m from launch param')
    ...
    def _capture_keyframe(self, yaw):
        if self.latest_rgb is None or self.latest_depth is None:
            return
        # Require fresh frames received AFTER settle started —
        # prevents capturing stale images from mid-flight.
        if not getattr(self, '_rgb_fresh', False) or \
                not getattr(self, '_depth_fresh', False):
            return
        self._rgb_fresh = False
        ...
        # Read EKF position variance (from VehicleOdometry)
        if self.odom is not None:
            pos_var = np.array(self.odom.position_variance[:3],
                               dtype=np.float32)
        else:
            pos_var = np.zeros(3, dtype=np.float32)

        # Commanded position (where NBV told the drone to go)
        if self.nbv_target is not None:
            cmd_pos = list(self.nbv_target['position'])
        else:
            cmd_pos = [px, py, pz]  # seed orbit — no separate command
        pos_error = float(math.sqrt(
            (px - cmd_pos[0])**2 + (py - cmd_pos[1])**2 + (pz - cmd_pos[2])**2))
        ...
        self.frames_meta.append({
            ...
            'commanded_ned': [float(v) for v in cmd_pos],
            'position_error_m': pos_error,
            'heading': yaw,
            'gimbal_pitch': self.gimbal_pitch_rad,
            'position_variance': [float(v) for v in pos_var],
        })
        ...
        sigma = np.sqrt(pos_var[:2].mean()) if pos_var[:2].sum() > 0 else 0.0
        self.get_logger().info(
            ...
            f'σ={sigma:.3f}m drift={pos_error:.3f}m '
    ...
    def _optimizer_loop(self):
        ...
        while rclpy.ok():
            ...
            # Scale iters with view count: ~50 passes/view, floor 750, cap 2500.
            ...
            # Shuffle views each epoch — baseline config (run 072).
            iter_losses = []
    ...
    def _loop(self):
        ...
        # ── TAKEOFF ──
            ...
            if self._dist_to(0.0, 0.0, takeoff_alt) < 0.5:
                self._enter_phase('DETECT_ABOVE')
                self._detect_settle = None
                self._detect_data = {}  # store results from both views
                self.gimbal_pitch_rad = math.radians(-90)
                self.get_logger().info('DETECT: phase 1 — flying above rock')

        # ── DETECT_ABOVE: overhead view to get x, y center ──
        elif self.phase == 'DETECT_ABOVE':
            detect_alt = -3.5
            self._pub_setpoint(
                self.rock_x, self.rock_y, detect_alt,
                math.atan2(self.rock_y, self.rock_x))

            if self._dist_to(self.rock_x, self.rock_y, detect_alt) < 0.5:
                if self._detect_settle is None:
                    self._detect_settle = self._sim_sec()
                    self._rgb_fresh = False
                    self._depth_fresh = False
                    self.get_logger().info('DETECT_ABOVE: arrived, settling')

                elapsed = self._sim_sec() - (self._detect_settle or self._sim_sec())
                if elapsed >= 1.5 and self.latest_depth is not None and \
                        getattr(self, '_depth_fresh', False):
                    depth_np = self.bridge.imgmsg_to_cv2(
                        self.latest_depth, 'passthrough')
                    if depth_np.shape[:2] == (H, W):
                        # Get x, y from overhead + top z + ground z
                        self._detect_from_above(depth_np)
                    # Move to side view
                    self._detect_settle = None
                    side_x = self._detect_data.get('cx', self.rock_x) + self.radius
                    side_y = self._detect_data.get('cy', self.rock_y)
                    side_z = self._detect_data.get('z_top', self.rock_ned[2]) - 0.3
                    self._detect_side_pos = (side_x, side_y, side_z)
                    self.gimbal_pitch_rad = math.radians(-15)
                    self._enter_phase('DETECT_SIDE')
                    self.get_logger().info(
                        f'DETECT: phase 2 — flying to side view at '
                        f'({side_x:.1f}, {side_y:.1f}, {side_z:.1f})')

        # ── DETECT_SIDE: side view to get vertical extent ──
        elif self.phase == 'DETECT_SIDE':
            sx, sy, sz = self._detect_side_pos
            yaw = math.atan2(
                self._detect_data.get('cy', self.rock_y) - sy,
                self._detect_data.get('cx', self.rock_x) - sx)
            self._pub_setpoint(sx, sy, sz, yaw)

            if self._dist_to(sx, sy, sz) < 0.5:
                if self._detect_settle is None:
                    self._detect_settle = self._sim_sec()
                    self._rgb_fresh = False
                    self._depth_fresh = False
                    self._set_gimbal_to_rock()
                    self.get_logger().info('DETECT_SIDE: arrived, settling')
                ...
                if elapsed >= 1.5 and self.latest_depth is not None and \
                    ...
                    depth_np = self.bridge.imgmsg_to_cv2(
                        self.latest_depth, 'passthrough')
                    if depth_np.shape[:2] == (H, W):
                        self._detect_from_side(depth_np)

                    # Finalize bbox from both views
                    self._finalize_detection()
                    # Publish bbox and continue to seed orbit
                    self.rviz.publish_bounding_box(
                        self.rock_ned.tolist(), self.bbox_size)
                    self._setup_pass(0)
                    self.waypoints = []
                    n_cardinal = min(4, self.seed_kfs)
                    for i in range(n_cardinal):
                        angle = 2.0 * math.pi * i / n_cardinal
                        x = self.rock_x + self.radius * math.cos(angle)
                        y = self.rock_y + self.radius * math.sin(angle)
                        z = self.alt_ned
                        yw = math.atan2(self.rock_y - y, self.rock_x - x)
                        self.waypoints.append((x, y, z, yw))
                    # Extra seeds if configured
                    if self.seed_kfs > 4:
                        overhead_alt = self.rock_ned[2] - self.radius * 0.9
                        overhead_r = self.radius * 0.5
                        for i, az_deg in enumerate([45, 225]):
                            if len(self.waypoints) >= self.seed_kfs:
                                break
                            az = math.radians(az_deg)
                            x = self.rock_x + overhead_r * math.cos(az)
                            y = self.rock_y + overhead_r * math.sin(az)
                            z = overhead_alt
                            yw = math.atan2(self.rock_y - y, self.rock_x - x)
                            self.waypoints.append((x, y, z, yw))
        ...
        # ── SEED: capture initial views to bootstrap 3DGS ──
            ...
            if self._dist_to(x, y, z) < 0.5:
                ...
                if not yaw_ok:
                    ...
                    # Mark frames stale — only capture fresh ones after settle
                    self._rgb_fresh = False
                    self._depth_fresh = False
